...
% =========================================================
% 频域方法在计算机视觉模型中的应用综述草稿：单栏版
% 编译建议：xelatex frequency_vision_review_single_column.tex
% =========================================================

% ---- 中文与基础排版 ----
\usepackage[UTF8]{ctex}
\usepackage{geometry}
\geometry{a4paper, left=2.6cm, right=2.6cm, top=2.5cm, bottom=2.5cm}
\usepackage{xcolor}
...
\title{\bfseries 频域方法在计算机视觉模型中的应用综述：\\ 表示、架构、鲁棒性与生成建模}
\author{你的名字\\你的单位\\邮箱：your.email@example.com}
\date{\today}
...
\noindent\textbf{关键词：} 频域分析；计算机视觉；傅里叶变换；小波变换；视觉 Transformer；鲁棒性；生成模型；文献综述
\end{abstract}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.96\textwidth]{figures/frequency_spectrum_semantics.pdf}
    \caption{低频与高频信息的视觉语义示意。低频分量通常对应整体轮廓、光照与全局布局，高频分量更常对应边缘、纹理、噪声和细粒度细节。}
    \label{fig:spectrum-semantics}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/review_workflow.pdf}
    \caption{频域视觉综述的写作工作流。从研究问题定义、文献检索与筛选，到分类体系构建、证据提取和写作迭代，形成可反复更新的闭环流程。}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/frequency_taxonomy.pdf}
    \caption{频域方法在计算机视觉中的分类框架。该框架同时关注频域信息在模型中的角色，以及不同视觉任务中的应用场景。}
    \label{fig:frequency-taxonomy}
\end{figure}
...
\section{代表性文献对比框架}
表~\ref{tab:taxonomy} 给出了后续填充文献时可使用的单栏表格模板。若后续文献数量较多，可以改用横向页面或拆分为多个表格。
...
\vspace{0.5em}
\small
\begin{tabular}{@{}p{2.4cm}p{3.2cm}p{3.0cm}p{3.6cm}@{}}
\toprule
\textbf{类别} & \textbf{频域工具} & \textbf{插入位置} & \textbf{主要收益与任务} \\ \midrule
频域表示 & FFT / DCT / Wavelet & 输入或中间特征 & 显式频谱建模；分类、复原、检测 \\
频域模块 & 谱滤波 / 频率注意力 & 网络 block & 全局建模、降复杂度；分类、分割、Transformer \\
频域正则 & 频带约束 / 频谱损失 & 损失函数或训练策略 & 鲁棒性、泛化性；域泛化、对抗鲁棒 \\
低级视觉 & 小波 / 多尺度频带 & 编码器--解码器 & 细节恢复、噪声抑制；去噪、超分、去模糊 \\
生成模型 & 频谱分析 / 频域控制 & 生成过程或判别器 & 伪影检测、纹理控制；GAN、扩散模型 \\ \bottomrule
\end{tabular}
\end{table}
